...
if not _FORCE_FALLBACK:
    try:
        from kernel_tilelang_original import (  # type: ignore
            act_quant as _tilelang_act_quant,
            fp4_act_quant as _tilelang_fp4_act_quant,
            fp8_gemm as _tilelang_fp8_gemm,
            fp4_gemm as _tilelang_fp4_gemm,
            sparse_attn as _tilelang_sparse_attn,
            hc_split_sinkhorn as _tilelang_hc_split_sinkhorn,
        )
        _USE_TILELANG = True
    except Exception:
        _USE_TILELANG = False
...
def act_quant(
    ...
    if _USE_TILELANG:
        return _tilelang_act_quant(x, block_size, scale_fmt, scale_dtype, inplace)
...
def fp4_act_quant(
    ...
    if _USE_TILELANG:
        return _tilelang_fp4_act_quant(x, block_size, inplace)
...
def fp8_gemm(
    ...
    if _USE_TILELANG:
        return _tilelang_fp8_gemm(a, a_s, b, b_s, scale_dtype)
...
def fp4_gemm(
    ...
    if _USE_TILELANG:
        return _tilelang_fp4_gemm(a, a_s, b, b_s, scale_dtype)
...
def hc_split_sinkhorn(
    ...
    if _USE_TILELANG:
        return _tilelang_hc_split_sinkhorn(mixes, hc_scale, hc_base, hc_mult, sinkhorn_iters, eps)
