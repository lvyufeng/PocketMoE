...
_EXT_PATH = Path(__file__).with_name("moe_dispatch_cuda_ext.so")
_NATIVE_MOD = None
...
def _find_built_extension() -> Optional[Path]:
    ...
    for suffix in suffixes:
        candidate = Path(__file__).with_name(f"moe_dispatch_cuda_ext{suffix}")
        if candidate.exists():
            return candidate
    return None
...
def load_moe_dispatch_cuda_ext() -> Optional[object]:
    ...
    try:
        _preload_torch_libs()
        spec = importlib.util.spec_from_file_location("moe_dispatch_cuda_ext", ext_path)
